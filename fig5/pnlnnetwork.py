...
ach_mat = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/ach_mat.npy")
...
g_ach = [0.0]*p_n+[2*90*0.5*0.1*0.05]*l_n         # Ach Conductance
E_ach = [0.0]*n_n                    # Ach Potential

## Defining GABAa Synapse Connectivity ##

fgaba_mat = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/fgaba_mat.npy")
...
g_fgaba = [0.3*6*1.2]*p_n+[30*0.2/2*1.2]*l_n #0.4       # fGABA Conductance
E_fgaba = [-70.0]*n_n                # fGABA Potential

## Defining GABAslow Synapse Connectivity ##

sgaba_mat = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/sgaba_mat.npy")
...
G_sgaba = [0.3*6*0.03]*p_n+[0.0]*l_n      # sGABA Conductance
...
current_input = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/current_input.npy")
state_vector = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/state_vector.npy")
t = np.load(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/timepoint.npy")
t_batch = np.array_split(t,4)
...
for n,i in tqdm(enumerate(t_batch)):
    ...
    states.append(state[:-1:int(1/sim_res),:120])

print("Completed. Total Execution Time:",np.round(time.time()-t_,3),"secs")

state = np.concatenate(states)

np.save(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/output_{sys.argv[4]}.npy",state)
np.save(f"__simcache__/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}/state_vector.npy",state_vector)
